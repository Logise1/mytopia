...
function drawSign(x, y) {
    if (signAsset.complete && signAsset.naturalWidth > 0) {
        ctx.drawImage(signAsset, x, y, 64, 64);
    } else {
        // Fallback simple si no carga el asset
        ctx.fillStyle = '#8b4513';
        ctx.fillRect(x + 28, y + 40, 8, 24); 
        ctx.fillStyle = '#deb887';
        ctx.fillRect(x + 8, y + 8, 48, 32);
    }
}
